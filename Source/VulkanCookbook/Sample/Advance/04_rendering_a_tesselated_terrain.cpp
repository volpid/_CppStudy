...
//----------------------------------------------------------------
// SampleAdvance04 : rendering a tesselated terrain
//----------------------------------------------------------------
class SampleAdvance04 : public VulkanSample
{
public:
...
private: 
    ...
    VkDestroyer(VkImage) _heightMap;
    VkDestroyer(VkDeviceMemory) _heightMapMemory;
    VkDestroyer(VkImageView) _heightMapView;
    VkDestroyer(VkSampler) _heightSampler;
    ...
    std::array<VkDestroyer(VkPipeline), 2> _graphicsPipeline;
};

//VULKAN_SAMPLE_FRAMEWORK("04_rendering_a_tesselated_terrain", 50, 25, 800, 600, SampleAdvance04);

//----------------------------------------------------------------
bool SampleAdvance04::Initialize(WindowParameters parameters)
{   
    _currentPipeline = 0;
    _camera = OrbitingCamera(Vector3{0.0f, 0.6f, 0.0f}, 4.0f);

    VkPhysicalDeviceFeatures deviceFeature = {};
    deviceFeature.tessellationShader = true;
    deviceFeature.geometryShader = true;
    deviceFeature.fillModeNonSolid = true;
    if (InitializeVulkan(parameters, &deviceFeature) == false)
    {
        return false;
    }

    if (Cookbook::Load3DModelFromObjFile("../../Resource/VulkanCookbook/Model/plane.obj", false, true, false, false, _model) == false)
    {
        return false;
    }
    ...
    if (Cookbook::LoadTextureDataFromFile("../../Resource/VulkanCookbook/Texture/heightmap.png", 4, imageData, &width, &height) == false)
    {
        return false;
    }

    InitVkDestroyer(_logicalDevice, _heightSampler);
    InitVkDestroyer(_logicalDevice, _heightMap);
    InitVkDestroyer(_logicalDevice, _heightMapMemory);
    InitVkDestroyer(_logicalDevice, _heightMapView);
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Advance\\04_shader.vert.spv", vertexShaderSpirv) == false)
    {
        return false;
    }
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Advance\\04_shader.tesc.spv", tessellationControlShaderSpirv) == false)
    {
        return false;
    }

    VkDestroyer(VkShaderModule) tessellationControlShaderModule;
    InitVkDestroyer(_logicalDevice, tessellationControlShaderModule);
    if (Cookbook::CreateShader(*_logicalDevice, tessellationControlShaderSpirv, *tessellationControlShaderModule) == false)
    {
        return false;
    }

    std::vector<unsigned char> tessellationEvaluationShaderSpirv;
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Advance\\04_shader.tese.spv", tessellationEvaluationShaderSpirv) == false)
    {
        return false;
    }

    VkDestroyer(VkShaderModule) tessellationEvaluationShaderModule;
    InitVkDestroyer(_logicalDevice, tessellationEvaluationShaderModule);
    if (Cookbook::CreateShader(*_logicalDevice, tessellationEvaluationShaderSpirv, *tessellationEvaluationShaderModule) == false)
    {
        return false;
    }

    std::vector<unsigned char> geometryShaderSpirv;
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Advance\\04_shader.geom.spv", geometryShaderSpirv) == false)
    {
        return false;
    }
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Advance\\04_shader.frag.spv", fragmentShaderSpirv) == false)
    {
        return false;
    }
    ...
    VkPipelineRasterizationStateCreateInfo soildRasterizationStateCreateInfo;
    Cookbook::SpecifyPipelineRasterizationState(false, 
        ...
        0.0f,
        1.0f,
        soildRasterizationStateCreateInfo);
    ...
    Cookbook::SpecifyPipelineRasterizationState(false, 
        ...
        0.0f,
        1.0f,
        lineRasterizationStateCreateInfo);
    ...
    InitVkDestroyer(_logicalDevice, _graphicsPipeline[0]);
    *_graphicsPipeline[0] = graphicsPipeline[0];

    graphicsPipelineCreateInfo.pRasterizationState = &lineRasterizationStateCreateInfo;
    if (Cookbook::CreateGraphicsPipeline(*_logicalDevice, {graphicsPipelineCreateInfo}, VK_NULL_HANDLE, graphicsPipeline) == false)
    {
        return false;
    }

    InitVkDestroyer(_logicalDevice, _graphicsPipeline[1]);
    *_graphicsPipeline[1] = graphicsPipeline[0];

    return true;
}

bool SampleAdvance04::Draw(void)
{
    auto prepareFrame = [this](VkCommandBuffer commandBuffer, uint32_t swapchainImageIndex, VkFramebuffer framebuffer) -> bool
    {
        ...
        Cookbook::BindPipelineObject(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *_graphicsPipeline[_currentPipeline]);
...
bool SampleAdvance04::Resize(void)
{
...
void SampleAdvance04::OnMouseEvent(void)
{
    _UpdateUniformbuffer(false);

    if (_mouseState.buttons[1].wasClicked == true)
    {
        _currentPipeline = 1 - _currentPipeline;
    }
}

bool SampleAdvance04::_UpdateUniformbuffer(bool force)
{
