...
//----------------------------------------------------------------
// SampleChapter06 : using uniform buffer
//----------------------------------------------------------------
class SampleChapter06 : public VulkanSample
{
public:
...
//VULKAN_SAMPLE_FRAMEWORK("06_using_uniform_buffers", 50, 25, 800, 600, SampleChapter06);

//----------------------------------------------------------------
bool SampleChapter06::Initialize(WindowParameters parameters)
{   
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Basic\\06_shader.vert.spv", vertexShaderSpirv) == false)
    {
        return false;
    }
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Basic\\06_shader.frag.spv", fragmentShaderSpirv) == false)
    {
        return false;
    }
    ...
    *_graphicsPipeline = graphicsPipelines[0];
    ...
    if (Cookbook::Load3DModelFromObjFile("../../Resource/VulkanCookbook/Model/cube.obj", false, false, false, true, _cube) == false)
    {
        return false;
    }
    ...
    if (Cookbook::CreateBuffer(*_logicalDevice, 
        sizeof(_cube.data[0]) * _cube.data.size(),
...
bool SampleChapter06::Draw(void)
{
...
bool SampleChapter06::Resize(void)
{
...
bool SampleChapter06::_UpdateStagingBuffer(void)
{
    Matrix4x4 rotationMatrix = Cookbook::PrepareRotationMatrix(40.0f, {0.0f, -1.0f, 0.0f});
    Matrix4x4 translationMatrix = Cookbook::PrepareTranslationMatrix(0.0f, 0.0f, -3.5f);
    Matrix4x4 perspectiveMatrix = Cookbook::PreparePerspectiveProjectionMatrix(static_cast<float>(_swapchain.size.width) / static_cast<float>(_swapchain.size.height), 30.0f, 1.0f, 10.0f);
    Matrix4x4 mpvMatrix = perspectiveMatrix * translationMatrix * rotationMatrix;

    if (Cookbook::UseStagingBufferToUpdateBufferWithDeviceLocalMemoryBound(_physicalDevice,
       *_logicalDevice,
       sizeof(mpvMatrix[0]) * mpvMatrix.size(),
       &mpvMatrix[0], 
       *_uniformBuffer, 
       0,
       0,
