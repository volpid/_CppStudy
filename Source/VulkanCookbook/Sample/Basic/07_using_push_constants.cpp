...
//----------------------------------------------------------------
// SampleChapter07 : using push constants
//----------------------------------------------------------------
class SampleChapter07 : public VulkanSample
{
public:
...
//VULKAN_SAMPLE_FRAMEWORK("07_using_push_constants", 50, 25, 800, 600, SampleChapter07);

//----------------------------------------------------------------
bool SampleChapter07::Initialize(WindowParameters parameters)
{   
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Basic\\07_shader.vert.spv", vertexShaderSpirv) == false)
    {
        return false;
    }
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Basic\\07_shader.frag.spv", fragmentShaderSpirv) == false)
    {
        return false;
    }
    ...
    if (Cookbook::CreatePipelineLayout(*_logicalDevice, {}, {pushConstantRange}, *_pipelineLayout) == false)
    {
        return false;
    }
...
bool SampleChapter07::Draw(void)
{
    auto prepareFrame = [this](VkCommandBuffer commandBuffer, uint32_t swapchainImageIndex, VkFramebuffer framebuffer) -> bool
    {
        ...
        Cookbook::ProvideDataToShaderThroughPushConstants(commandBuffer,
            *_pipelineLayout,
            VK_SHADER_STAGE_FRAGMENT_BIT,
            0,
            static_cast<uint32_t>(sizeof(color[0]) * color.size()), 
            &color[0]);
...
bool SampleChapter07::Resize(void)
{
