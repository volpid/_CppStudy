...
//----------------------------------------------------------------
// SampleLighting03 : rendering a normal mapped geometry
//----------------------------------------------------------------
class SampleLighting03 : public VulkanSample
{
public:
...
//VULKAN_SAMPLE_FRAMEWORK("03_rendering_a_normal_mapped_geometry", 50, 25, 800, 600, SampleLighting03);

//----------------------------------------------------------------
bool SampleLighting03::Initialize(WindowParameters parameters)
{   
    ...
    if (Cookbook::LoadTextureDataFromFile("../../Resource/VulkanCookbook/Texture/normal_map.png", 4, imageData, &width, &height) == false)
    {
        return false;
    }
    ...
    if (Cookbook::Load3DModelFromObjFile("../../Resource/VulkanCookbook/Model/ice.obj", true, true, true, true, _model, &vertexStride) == false)
    {
        return false;
    }
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Lighting\\03_shader.vert.spv", vertexShaderSpirv) == false)
    {
        return false;
    }
    ...
    if (GetBinaryFileContent("..\\..\\Source\\VulkanCookbook\\Shader\\Lighting\\03_shader.frag.spv", fragmentShaderSpirv) == false)
    {
        return false;
    }
...
bool SampleLighting03::Draw(void)
{
    auto prepareFrame = [this](VkCommandBuffer commandBuffer, uint32_t swapchainImageIndex, VkFramebuffer framebuffer) -> bool
    {
        ...
        std::array<float, 4> lightPosition = {5.0f, 5.0f, 1.0f, 0.0f};        
        Cookbook::ProvideDataToShaderThroughPushConstants(commandBuffer, 
            *_graphicsPipelineLayout,
            VK_SHADER_STAGE_FRAGMENT_BIT,
            0,
            sizeof(float) * lightPosition.size(),
            lightPosition.data());
...
bool SampleLighting03::Resize(void)
{
...
void SampleLighting03::OnMouseEvent(void)
{
    _UpdateUniformbuffer(false);
}

bool SampleLighting03::_UpdateUniformbuffer(bool force)
{
